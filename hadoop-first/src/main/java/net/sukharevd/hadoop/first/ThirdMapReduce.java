...
public final class ThirdMapReduce extends Configured implements Tool {
	...
	public int run(String[] args) throws Exception {
		...
		JobConf job = new JobConf(conf, ThirdMapReduce.class);
		Path in = new Path(args[0]);
		Path out = new Path(args[1]);
		FileInputFormat.setInputPaths(job, in);
		FileOutputFormat.setOutputPath(job, out);
		job.setJobName("HistogramPatentMapReduceJob");
	...
	public static void main(String[] args) throws Exception {
		int res = ToolRunner.run(new Configuration(), new ThirdMapReduce(), args);
		System.exit(res);
	}
